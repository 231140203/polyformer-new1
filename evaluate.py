...
def main(cfg: DictConfig, **kwargs):
    ...
    for sample in progress:
        ...
        # =================================================================
        # 【增加】遍历当前 Batch，保存每张切片的 Dice
        # =================================================================

        # 1. 提取当前批次所有的 ID
        batch_ids = sample['id'].tolist() if torch.is_tensor(sample['id']) else sample['id']
        
        # 2. 追加写入 CSV 文件
        with open('per_slice_dice.csv', 'a', newline='') as f:
            writer = csv.writer(f)
            
            # 3. 把这个 Batch (批次) 里的图片挨个拿出来算
            for i in range(len(batch_ids)):
                uniq_id = batch_ids[i]
                
                # 提取单张图片的交集(I)和并集(U)
                I = cum_I[i].item() if torch.is_tensor(cum_I[i]) else cum_I[i]
                U = cum_U[i].item() if torch.is_tensor(cum_U[i]) else cum_U[i]

                # 严格只计算前景的 Dice
                single_dice = 1.0 if U == 0 else (2 * I) / (I + U)

                # 写入成绩单: [图片ID, Dice分数]
                writer.writerow([uniq_id, single_dice])
        # =================================================================
